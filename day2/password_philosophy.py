...
For example, 1-3 a means that the password must 
...
def part1(data):
    ...
    for lower, upper, char, string in data:
        ...
        if char in counter and (lower <= counter[char] <= upper):
            ans += 1
    return ans
...
and so on. (Be careful; Toboggan Corporate Policies have 
no concept of "index zero"!) Exactly one of these positions 
